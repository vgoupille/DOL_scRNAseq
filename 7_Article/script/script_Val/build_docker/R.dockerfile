...
# Créer un fichier myenv_R.yml avec toutes les dépendances
COPY myenv_R.yml /tmp/myenv_R.yml

# Créer l'environnement Conda depuis le fichier myenv_R.yml
RUN conda env create -f /tmp/myenv_R.yml && \
    rm /tmp/myenv_R.yml

# Activer l'environnement Conda 'myenv_R' par défaut
RUN echo "source activate myenv_R" > ~/.bashrc

#Installer les packages R nécessaires depuis un fichier 
